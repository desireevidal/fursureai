...
class AudioPreprocessor {
  ...
  Float32List _trimSilence(Float32List audio) {
    ...
    double maxRms = 0.0;
    final rmsDb = Float64List(numFrames);
    ...
    for (var f = 0; f < numFrames; f++) {
      if (20.0 * math.log(math.max(_amin, rmsDb[f])) / math.ln10 >= threshold) {
        startFrame = f;
        break;
      }
    }
    ...
    return Float32List.sublistView(audio, startSample, endSample);
  }
  ...
  List<Float64List> _buildMelFilterbank() {
    ...
    final fdiff = List.generate(_nMels + 1, (i) => hzPoints[i + 1] - hzPoints[i]);
